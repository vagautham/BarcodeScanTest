...
- (id) initWithFrame:(CGRect)theFrame cancelEnabled:(BOOL)isCancelEnabled oneDMode:(BOOL)isOneDModeEnabled showLicense:(BOOL)showLicenseButton {
  ...
  if( self ) {
    ...
    CGFloat rectSize = self.frame.size.width - kPadding * 4;
    if (!oneDMode) {
        int ht = 100;
        cropRect = CGRectMake(self.frame.size.width/2.0- rectSize/2.0, self.frame.size.height/2.0-ht/2.0, rectSize, ht);
    } else {
...
//      [self addSubview:cancelButton];
        ...
        cancelButton.frame = CGRectMake(0, self.frame.size.height-65.0, self.frame.size.width, 65.0);
        cancelButton.backgroundColor = [UIColor colorWithRed:240.0/255.0 green:22.0/255.0 blue:22.0/255.0 alpha:1.0];
        [cancelButton setTitle:@"Cancel" forState:UIControlStateNormal];
        [cancelButton setTitle:@"Cancel" forState:UIControlStateDisabled];
...
//    self.displayedMessage = NSLocalizedStringWithDefaultValue(@"OverlayView displayed message", nil, [NSBundle mainBundle], @"Place a barcode inside the viewfinder rectangle to scan it.", @"Place a barcode inside the viewfinder rectangle to scan it.");
  ...
  //	CGContextSetStrokeColor(c, white);
    ...
    CGRect displayRect = CGRectMake((rect.size.width - displaySize.width) / 2 , cropRect.origin.y - displaySize.height - 3.0, displaySize.width, displaySize.height);
	...
	if (oneDMode) {
		...
		CGContextSetFillColor(c, red);
		CGContextBeginPath(c);
		CGContextMoveToPoint(c, cropRect.origin.x-kPadding, cropRect.origin.y+cropRect.size.height/2.0);
		CGContextAddLineToPoint(c, cropRect.origin.x+cropRect.size.width+kPadding, cropRect.origin.y+cropRect.size.height/2.0);
		CGContextStrokePath(c);
        CGContextRestoreGState(c);
    }
	if( nil != _points ) {
